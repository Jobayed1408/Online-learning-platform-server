  ...
  try {
    ...
    app.post("/enroll", async (req, res) => {
      ...
      try {
        ...
        if ((course.created_by) && (course.created_by === userEmail))
          return res.send({
            success: false,
            message: "You cannot enrolled your own course!!"
          });
    ...
    app.put("/update-course/:id", async (req, res) => { 
      ...
      try {
        ...
        if (result.modifiedCount > 0) {
          ...
          res.send({ success: false, message: "No change detected for update" });
        }
      } catch (error) {
